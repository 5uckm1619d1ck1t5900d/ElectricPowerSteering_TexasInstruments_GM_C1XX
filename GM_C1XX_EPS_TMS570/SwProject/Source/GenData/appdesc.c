/* -----------------------------------------------------------------------------
  ...
  Description: Implementation example for the proper usage with CANdesc.
                ...
                EcuDocFile:   C:\CMSynergy_Projects\Chrysler_C1xx\Chrysler_LWR_EPS_TMS570\Tools\AsrProject\GENy\EPS_5.0.3_HS_60.cdd
                Variant:      CommonDiagnostics

  Generated by CANdelaGen, Wed Oct 28 11:15:34 2015
...
/* 3673 - The object addressed by the pointer parameter is not modified and so the pointer could be of type 'pointer to const' */
...
#include "CDD_Data.h" /* Provide access to CDD_DemIdSnapshot_Cnt_G_u08 */
...
/* MISRA-C:2004 Rule 8.8: Deviation allowed in Type 1A files because of the configurable data. For example in Adc2.c - Module internal configuration data structure, post build configuration of this module not currently supported. Thus the configuration structure is defined within the static source and initialized with values from the configuratin file. Deviation allowed on case basis with proper justification in comments. */
/* MISRA-C:2004 Rule 10.3: Library Exception. */
/* MISRA-C:2004 Rule 10.4: Library Exception. */
/* MISRA-C:2004 Rule 11.3: This deviation is required for register access.  If there is frequency of usage throughout the file then rule can be suppressed for the entire file. Annotation at each deviation unecessarily clutters the file. */
/* MISRA-C:2004 Rule 12.4: Careful analysis needs to be done and all possible combinations needs to be analyzed for desired results. Warning can be suppressed after careful analysis. We will keep this warning ON. */
/* MISRA-C:2004 Rule 14.2: Deviation allowed in current guideline. Currently these statements are of no use in the code and can be easily removed without any side effect on the functionality, however if they are intended to be implemented in future (which looks like the possibility), then suppress the warnings associate with them. Configurable Start of Runnable Checkpoint  or  Configurable Stop of Runnable Checkpoint  or Metrics start hook does nothing in production build. or  Metrics end hook does nothing in production build. */
...
/* MISRA-C:2004 Rule 21.1: Deviation allowed in current guideline. */
...
#define D_ECUINTERNALTEMPLOLMT_DEGC_F32		(-40.0)
#define D_ECUINTERNALTEMPHILMT_DEGC_F32		215.0
...
#define D_RESETDEACTIVATE_CNT_U16			0x00U
#define D_RESETALL_CNT_U16					0x01U
#define D_RESETAPC_CNT_U16					0x02U
#define D_RESETEOT_CNT_U16					0x03U
#define D_RESETTSMIT_CNT_U16				0x04U
...
#define D_SSSTATELOWLIMIT_CNT_U08			0U
#define D_SSSTATEHIGHLIMIT_CNT_U08			5U
...
STATIC VAR(uint16, APPDESC_VAR_NOINIT) BadKeyCounter_Cnt_M_u16;
...
FUNC(void, RTE_APPDESC_APPL_CODE) AppDesc_Init(void)
{
	...
	for(Index_Cnt_T_u8 = 0U; Index_Cnt_T_u8 < (uint8)D_NUMOFDEMEVENTS_CNT_U08; Index_Cnt_T_u8++)
	{
 ...
 *	Outputs:	       None
 *****************************************************************************/
FUNC(void, RTE_APPDESC_APPL_CODE) AppDesc_Per1(void)		/* PRQA S 3408 */
{
	...
	if (PerformDigColPsCustSetTrim_Cnt_M_lgc == TRUE)
	{
		if (DigColPsCustSetTrimLoopCntr_Cnt_M_u08 == 0U)
		{
			...
			else
			{
				...
				RespMsgBuffer[2] = (DescNegResCode)kDescNrcDeviceControlLimitExceeded;
...
void DESC_API_CALLBACK_TYPE ApplDescFatalError(vuint8 errorCode, vuint16 lineNumber)
{
  ...
  /* Avoid warnings */
  DESC_IGNORE_UNREF_PARAM(errorCode);		/* PRQA S 3112 */
  DESC_IGNORE_UNREF_PARAM(lineNumber);		/* PRQA S 3112 */
...
void DESC_API_CALLBACK_TYPE ApplDescClearDiagnosticTroubleCodes(DescMsgContext* pMsgContext)
{
	...
	if ((DescGetStateSecurity_Access() != kDescStateSecurity_AccessCPID_Unlock) &&
		...
		(Rte_SrlComInput_SrlComVehSpd_Kph_f32 > k_ClearDtcVehSpdLmt_Kph_f32)))
	{
	...
	while(DTCindex <= (uint8)D_NUMOFDEMEVENTS_CNT_U08)
	{
		(void)Dem_GetDTCOfEvent(DTCindex, (Dem_DTCKindType)DEM_DTC_KIND_ALL_DTCS, &DTCNum_Cnt_T_u32);
		(void)Dem_ClearDTC(DTCNum_Cnt_T_u32, (Dem_DTCKindType)DEM_DTC_KIND_ALL_DTCS, 
...
void DESC_API_CALLBACK_TYPE ApplDescReadByPIDFailure_Record_Data_Identifiers(DescMsgContext* pMsgContext)
{
	...
	VAR(uint8, AUTOMATIC) Evnt_Cnt_T_u8;
	VAR(uint8, AUTOMATIC) Index_Cnt_T_u8 = 0U;

	for(Evnt_Cnt_T_u8 = 1U; Evnt_Cnt_T_u8 <= (uint8)D_NUMOFDEMEVENTS_CNT_U08; Evnt_Cnt_T_u8++)
	{
		(void)Dem_GetEventStatus(Evnt_Cnt_T_u8, &ISOStatus_Cnt_T_u8);
		GMStatus_Cnt_T_u8 = ISOToGMStatus(ISOStatus_Cnt_T_u8);

		if (0U != (GMStatus_Cnt_T_u8 & D_SRVC12STATMSK_CNT_U8))
		{
			(void)Dem_GetDTCOfEvent(Evnt_Cnt_T_u8, (Dem_DTCKindType)DEM_DTC_KIND_ALL_DTCS, &FailRecDTCNum_Cnt_M_u32[Index_Cnt_T_u8]);
			Index_Cnt_T_u8++;
		}
	}


	pMsgContext->resData[0] = 0x00u; /* 0x00 = PID response */ /* PRQA S 0491 */

	for (Evnt_Cnt_T_u8 = 0U; Evnt_Cnt_T_u8 < Index_Cnt_T_u8; Evnt_Cnt_T_u8++)
	{
		pMsgContext->resData[(Evnt_Cnt_T_u8 * 4u) + 1u] = Evnt_Cnt_T_u8; /* PRQA S 0491 */
		pMsgContext->resData[(Evnt_Cnt_T_u8 * 4u) + 2u] = (uint8)(FailRecDTCNum_Cnt_M_u32[Evnt_Cnt_T_u8] >> D_WORDSHIFT_CNT_U16); /* PRQA S 0491 */
		pMsgContext->resData[(Evnt_Cnt_T_u8 * 4u) + 3u] = (uint8)(FailRecDTCNum_Cnt_M_u32[Evnt_Cnt_T_u8] >> D_BYTESHIFT_CNT_U16); /* PRQA S 0491 */
		pMsgContext->resData[(Evnt_Cnt_T_u8 * 4u) + 4u] = (uint8)((FailRecDTCNum_Cnt_M_u32[Evnt_Cnt_T_u8]) & 0x00FFU); /* PRQA S 0491 */
	}

	pMsgContext->resDataLen = (((DescMsgLen)Evnt_Cnt_T_u8) * 4U) + 1U;
...
void DESC_API_CALLBACK_TYPE ApplDescRFRD_RFRP(DescMsgContext* pMsgContext)
{
	...
	if(FailRecIndex_Cnt_T_u08 < (uint8)D_NUMOFDEMEVENTS_CNT_U08)
	{
		/* Check that the CTC was reported to have customer data when $12 $01 was called. */
		CTC_Cnt_T_u32 = (((uint32)pMsgContext->reqData[1] << D_WORDSHIFT_CNT_U16) | ((uint32)pMsgContext->reqData[2] << D_BYTESHIFT_CNT_U16) | pMsgContext->reqData[3]);		/* PRQA S 0491 */
		if (CTC_Cnt_T_u32 == FailRecDTCNum_Cnt_M_u32[FailRecIndex_Cnt_T_u08])
		{
			...
			DemRet_Cnt_T_enum = Dem_GetFreezeFrameDataByDTC(CTC_Cnt_T_u32, (uint8)DEM_DTC_KIND_ALL_DTCS, (uint8)DEM_DTC_ORIGIN_PRIMARY_MEMORY, 0x01u, DEM_GET_FFDATA_ID_ALL, FreezeFrame_Cnt_T_u08, &FreezeFrameSize_Cnt_T_u08);
			if(((Dem_ReturnGetFreezeFrameDataByDTCType)DEM_GET_FFDATABYDTC_OK != DemRet_Cnt_T_enum) || (FreezeFrameSize_Cnt_T_u08 == 0u))
			{
				...
				DemRet_Cnt_T_enum = Dem_GetFreezeFrameDataByDTC(CTC_Cnt_T_u32, (uint8)DEM_DTC_KIND_ALL_DTCS, (uint8)DEM_DTC_ORIGIN_PRIMARY_MEMORY, 0x00u, DEM_GET_FFDATA_ID_ALL, FreezeFrame_Cnt_T_u08, &FreezeFrameSize_Cnt_T_u08);
			}
...
{
	VAR(uint16, AUTOMATIC) Index_Cnt_T_u8 = 0u;
...
void DESC_API_CALLBACK_TYPE ApplDescRead_DID_98_Repair_Shop_Code_Or_Tester_Serial_Number(DescMsgContext* pMsgContext)
{
	...
	while (Index_Cnt_T_u8 < 10U)
	{
		pMsgContext->resData[Index_Cnt_T_u8] = Nvm_RepairShopCode_Cnt_u8[Index_Cnt_T_u8];		/* PRQA S 0491 */
		Index_Cnt_T_u8++;
	}
	/* Always set the correct length of the response data. */
...
void DESC_API_CALLBACK_TYPE ApplDescRead_DID_9F_Previous_Stored_Repair_Shop_Code_Or_Tester_Serial_Number(DescMsgContext* pMsgContext)
{
	...
	for (Index_Cnt_T_u8 = 0u; Index_Cnt_T_u8 < TableSize_m(Nvm_RepairShopCode_Cnt_u8); Index_Cnt_T_u8++)
	{
		pMsgContext->resData[Index_Cnt_T_u8] = Nvm_RepairShopCode_Cnt_u8[Index_Cnt_T_u8];		/* PRQA S 0491 */
	}

	for (Index_Cnt_T_u8 = 0u; Index_Cnt_T_u8 < TableSize_m(Nvm_PrevRepairShopCode_Cnt_u8); Index_Cnt_T_u8++)
	{
		pMsgContext->resData[Index_Cnt_T_u8 + TableSize_m(Nvm_RepairShopCode_Cnt_u8)] = Nvm_PrevRepairShopCode_Cnt_u8[Index_Cnt_T_u8];		/* PRQA S 0491 */
	}

	pMsgContext->resDataLen = TableSize_m(Nvm_RepairShopCode_Cnt_u8) + TableSize_m(Nvm_PrevRepairShopCode_Cnt_u8);
	DescProcessingDone(pMsgContext->iContext);
}
...
void DESC_API_CALLBACK_TYPE ApplDescRead_DID_C0_Boot_Software_Part_Number(DescMsgContext* pMsgContext)
{
	...
	while (Index_Cnt_T_u8 < 4U)
	{
		pMsgContext->resData[Index_Cnt_T_u8] = GetFblSWMI(Index_Cnt_T_u8);	/* PRQA S 0491 */ /* PRQA S 306++*/
		Index_Cnt_T_u8++;
	}
	pMsgContext->resDataLen = 4u;
	/* User service processing finished. */
...
{
	pMsgContext->resData[0] = ((vuint8)((GM_ApplHeader.PART_NO >> 24) & 0xFFU));	/* PRQA S 0491 */ /* PRQA S 2985 */
	...
	pMsgContext->resData[3] = ((vuint8)((GM_ApplHeader.PART_NO >> 0) & 0xFFU));		/* PRQA S 0491 */ /* PRQA S 2985 */
...
void DESC_API_CALLBACK_TYPE ApplDescRead_DID_D0_Boot_Software_Part_Number_Alpha_Code(DescMsgContext* pMsgContext)
{
	...
	pMsgContext->resData[0] = GetFblDLS0();		/* PRQA S 0491 */		/* PRQA S 306 ++ */
	pMsgContext->resData[1] = GetFblDLS1();		/* PRQA S 0491 */		/* PRQA S 306 ++  */
	/* Always set the correct length of the response data. */
...
{
	pMsgContext->resData[0] = DescGetHiByte(GM_ApplHeader.DLS);		/* PRQA S 0491 */
	pMsgContext->resData[1] = DescGetLoByte(GM_ApplHeader.DLS);		/* PRQA S 0491 */
...
{
	pMsgContext->resData[0] = DescGetHiByte(GM_CalHeader.DLS);		/* PRQA S 0491 */
	pMsgContext->resData[1] = DescGetLoByte(GM_CalHeader.DLS);		/* PRQA S 0491 */
...
{
	pMsgContext->resData[0] = DescGetHiByte(GM_Cal2Header.DLS);		/* PRQA S 0491 */
	pMsgContext->resData[1] = DescGetLoByte(GM_Cal2Header.DLS);		/* PRQA S 0491 */
	pMsgContext->resDataLen = 2u;
	DescProcessingDone(pMsgContext->iContext);
}
...
void DESC_API_CALLBACK_TYPE ApplDescRead_DID_F0_ProgrammedStateIndicator_PSI(DescMsgContext* pMsgContext)
{
	...
	pMsgContext->resData[0] = 0x01U;  /* Main processor Application SW Partition */ /* PRQA S 0491 */
	pMsgContext->resData[1] = 0x00U;  /* Valid programming present */               /* PRQA S 0491 */
	pMsgContext->resData[2] = 0x02U;  /* Main processor 1st Cal Partition */        /* PRQA S 0491 */
	pMsgContext->resData[3] = 0x00U;  /* Valid programming present */               /* PRQA S 0491 */
	pMsgContext->resData[4] = 0x03U;  /* Main processor 2nd Cal Partition */        /* PRQA S 0491 */
	pMsgContext->resData[5] = 0x00U;  /* Valid programming present */               /* PRQA S 0491 */
...
void DESC_API_CALLBACK_TYPE ApplDescReadMemory(DescMsgContext* pMsgContext)
{
	...
	Address_Cnt_T_u32 = pMsgContext->reqData[0]; 		/* PRQA S 0491 */
	pMsgContext->resData[0] = pMsgContext->reqData[0]; 	/* PRQA S 0491 */
	Address_Cnt_T_u32 <<= D_BYTESHIFT_CNT_U16;
	Address_Cnt_T_u32 += pMsgContext->reqData[1]; 		/* PRQA S 0491 */
	pMsgContext->resData[1] = pMsgContext->reqData[1]; 	/* PRQA S 0491 */
	Address_Cnt_T_u32 <<= D_BYTESHIFT_CNT_U16;
	Address_Cnt_T_u32 += pMsgContext->reqData[2]; 		/* PRQA S 0491 */
	pMsgContext->resData[2] = pMsgContext->reqData[2]; 	/* PRQA S 0491 */
	Address_Cnt_T_u32 <<= D_BYTESHIFT_CNT_U16;
	Address_Cnt_T_u32 += pMsgContext->reqData[3]; 		/* PRQA S 0491 */
	pMsgContext->resData[3] = pMsgContext->reqData[3]; 	/* PRQA S 0491 */
	/* get the memory length to read  */
	Length_Cnt_T_u16 = pMsgContext->reqData[4]; 		/* PRQA S 0491 */
	Length_Cnt_T_u16 <<= D_BYTESHIFT_CNT_U16;
	Length_Cnt_T_u16 += pMsgContext->reqData[5]; 		/* PRQA S 0491 */
	...
	else if ((Address_Cnt_T_u32 <= (uint32)(&_Hidden_Ram_End)) &&
	...
	else
	{
		...
		{
			TempRead_Cnt_T_u08 = *((uint8 *)(Address_Cnt_T_u32 + Index_Cnt_T_u16));
...
void DESC_API_CALLBACK_TYPE ApplDescRequestSPS_Seed(DescMsgContext* pMsgContext)
{
	...
	if( (D_SECURITYDELAYTIME_MS_U32 < TimeoutElapsedTime_mS_T_u32) || (FALSE == SecurityTimeArmed_Cnt_M_lgc) )
	{
		if (DescGetStateSecurity_Access() != kDescStateSecurity_AccessSPS_Unlock)
		{
			...
			pMsgContext->resData[0] = Nvm_SPSSeed_Cnt_str.Data.Seed[0U]; /* PRQA S 0491 */
			pMsgContext->resData[1] = Nvm_SPSSeed_Cnt_str.Data.Seed[1U]; /* PRQA S 0491 */
			pMsgContext->resData[2] = Nvm_SPSSeed_Cnt_str.Data.Seed[2U]; /* PRQA S 0491 */
			pMsgContext->resData[3] = Nvm_SPSSeed_Cnt_str.Data.Seed[3U]; /* PRQA S 0491 */
			pMsgContext->resData[4] = Nvm_SPSSeed_Cnt_str.Data.Seed[4U]; /* PRQA S 0491 */
		}
		else
		{
...
void DESC_API_CALLBACK_TYPE ApplDescSendSPS_Key(DescMsgContext* pMsgContext)
{
	...
	{
		SPSKeyAllowed_Cnt_M_lgc = FALSE;
		if ((pMsgContext->reqData[0U] != Nvm_SPSKey_Cnt_str.Data.Key[0U]) || /* PRQA S 0491 */
			(pMsgContext->reqData[1U] != Nvm_SPSKey_Cnt_str.Data.Key[1U]) || /* PRQA S 0491 */
			(pMsgContext->reqData[2U] != Nvm_SPSKey_Cnt_str.Data.Key[2U]) || /* PRQA S 0491 */
			(pMsgContext->reqData[3U] != Nvm_SPSKey_Cnt_str.Data.Key[3U]) || /* PRQA S 0491 */
			(pMsgContext->reqData[4U] != Nvm_SPSKey_Cnt_str.Data.Key[4U])) /* PRQA S 0491 */
		{
			BadKeyCounter_Cnt_M_u16 = BadKeyCounter_Cnt_M_u16 - 1U;
			if (BadKeyCounter_Cnt_M_u16 == 0U)
			{
				SecurityTimeArmed_Cnt_M_lgc = TRUE;
				GetSystemTime_mS_u32(&(SecurityTimerStart_mS_M_u32p0));
				BadKeyCounter_Cnt_M_u16 = 1U;
...
{
	VAR(uint32, AUTOMATIC) TimeoutElapsedTime_mS_T_u32;
	VAR(uint8, AUTOMATIC) Index_T_Cnt = 0U;
	...
	if( (D_SECURITYDELAYTIME_MS_U32 < TimeoutElapsedTime_mS_T_u32) || (FALSE == SecurityTimeArmed_Cnt_M_lgc) )
	{
		         ...
		         ((Nvm_CMEC_Cnt_u8 != 0x00U) && (Nvm_CMEC_Cnt_u8 != 0xFFU)))
		{
			...
			pMsgContext->resData[0] = Nvm_CPIDSeed_Cnt_str.Data.Seed[0U]; /* PRQA S 0491 */
			pMsgContext->resData[1] = Nvm_CPIDSeed_Cnt_str.Data.Seed[1U]; /* PRQA S 0491 */
			pMsgContext->resData[2] = Nvm_CPIDSeed_Cnt_str.Data.Seed[2U]; /* PRQA S 0491 */
			pMsgContext->resData[3] = Nvm_CPIDSeed_Cnt_str.Data.Seed[3U]; /* PRQA S 0491 */
			pMsgContext->resData[4] = Nvm_CPIDSeed_Cnt_str.Data.Seed[4U]; /* PRQA S 0491 */
		...
		else
		{
			...
			while (Index_T_Cnt < 5U)
			{	
				pMsgContext->resData[Index_T_Cnt] = 0x00U; /* PRQA S 0491 */
				Index_T_Cnt++;
 		  	}
			/* Set the correct length of the response data. */
	        pMsgContext->resDataLen = 5u;
...
void DESC_API_CALLBACK_TYPE ApplDescSendDevice_Control_Key(DescMsgContext* pMsgContext)
{
	...
	if (CPIDKeyAllowed_Cnt_M_lgc == TRUE)
	{
		...
		{
			if ((pMsgContext->reqData[0U] != Nvm_CPIDKey_Cnt_str.Data.Key[0U]) || /* PRQA S 0491 */
				(pMsgContext->reqData[1U] != Nvm_CPIDKey_Cnt_str.Data.Key[1U]) || /* PRQA S 0491 */
				(pMsgContext->reqData[2U] != Nvm_CPIDKey_Cnt_str.Data.Key[2U]) || /* PRQA S 0491 */
				(pMsgContext->reqData[3U] != Nvm_CPIDKey_Cnt_str.Data.Key[3U]) || /* PRQA S 0491 */
				(pMsgContext->reqData[4U] != Nvm_CPIDKey_Cnt_str.Data.Key[4U])) /* PRQA S 0491 */
			{
				BadKeyCounter_Cnt_M_u16 = BadKeyCounter_Cnt_M_u16 - 1U;
				if (BadKeyCounter_Cnt_M_u16 == 0U)
				{
...
void DESC_API_CALLBACK_TYPE ApplDescRequestNexteer_Seed(DescMsgContext* pMsgContext)
{
	...
	if( (D_SECURITYDELAYTIME_MS_U32 < TimeoutElapsedTime_mS_T_u32) || (FALSE == SecurityTimeArmed_Cnt_M_lgc) )
	{
		if (DescGetStateSecurity_Access() != kDescStateSecurity_AccessNexteer_Unlock)
		{
			...
			pMsgContext->resData[0] = Nvm_NexteerSeed_Cnt_str.Data.Seed[0U]; /* PRQA S 0491 */
			pMsgContext->resData[1] = Nvm_NexteerSeed_Cnt_str.Data.Seed[1U]; /* PRQA S 0491 */
			pMsgContext->resData[2] = Nvm_NexteerSeed_Cnt_str.Data.Seed[2U]; /* PRQA S 0491 */
			pMsgContext->resData[3] = Nvm_NexteerSeed_Cnt_str.Data.Seed[3U]; /* PRQA S 0491 */
			pMsgContext->resData[4] = Nvm_NexteerSeed_Cnt_str.Data.Seed[4U]; /* PRQA S 0491 */
		}
		else
		{
			DescActivateS1Timer();  /* start the tester present timer  */
...
void DESC_API_CALLBACK_TYPE ApplDescSendNexteer_Key(DescMsgContext* pMsgContext)
{
	...
	{
		NexteerKeyAllowed_Cnt_M_lgc = FALSE;
		if ((pMsgContext->reqData[0U] != Nvm_NexteerKey_Cnt_str.Data.Key[0U]) || /* PRQA S 0491 */
			(pMsgContext->reqData[1U] != Nvm_NexteerKey_Cnt_str.Data.Key[1U]) || /* PRQA S 0491 */
			(pMsgContext->reqData[2U] != Nvm_NexteerKey_Cnt_str.Data.Key[2U]) || /* PRQA S 0491 */
			(pMsgContext->reqData[3U] != Nvm_NexteerKey_Cnt_str.Data.Key[3U]) || /* PRQA S 0491 */
			(pMsgContext->reqData[4U] != Nvm_NexteerKey_Cnt_str.Data.Key[4U])) /* PRQA S 0491 */
		{
			BadKeyCounter_Cnt_M_u16 = BadKeyCounter_Cnt_M_u16 - 1U;
			if (BadKeyCounter_Cnt_M_u16 == 0U)
			{
	...
	{
		DescSetNegResponse(pMsgContext->iContext, (vuint8)kDescNrcRequestSequenceError);
	}
...
void DESC_API_CALLBACK_TYPE ApplDescRequestDownloadDownload(DescMsgContext* pMsgContext)
{
	...
	else
	{
		...
		/* ----------------------------------------------------------------*/
		CanInitTable_Cnt_M_str.requestDownloadMemorySize = ((vuint32)pMsgContext->reqData[3] << 8u) | /* PRQA S 0491 */
		((vuint32)pMsgContext->reqData[4]); /* PRQA S 0491 */
...
void DESC_API_CALLBACK_TYPE ApplDescWrite_DID_90_Vehicle_Identification_Number(DescMsgContext* pMsgContext)
{
	...
	{
		DescSetNegResponse(pMsgContext->iContext, (vuint8)kDescNrcRequestOutOfRange);
	}
	else
	{
		while (Index_Cnt_T_u8 < 17U)
		{
			Nvm_VIN_Cnt_u8[Index_Cnt_T_u8] = pMsgContext->reqData[Index_Cnt_T_u8]; /* PRQA S 0491 */
			Index_Cnt_T_u8++;
		}
...
{
	VAR(uint16, AUTOMATIC) Index_Cnt_T_u8;

	/* If the message is at least 1 character and at most the length of the NvM buffer... */
	if ((pMsgContext->reqDataLen >= (DescMsgLen)1U) &&
		(pMsgContext->reqDataLen <= (DescMsgLen)20U))
	{
		/* Copy the available request information to the NvM buffer. */
		for (Index_Cnt_T_u8 = 0U; Index_Cnt_T_u8 < pMsgContext->reqDataLen; Index_Cnt_T_u8++)
		{
			Nvm_SystemName_Cnt_u8[Index_Cnt_T_u8] = pMsgContext->reqData[Index_Cnt_T_u8]; /* PRQA S 0491 */
		}

		/* Fill the rest of the NvM buffer with null ('\0') characters. */
		for (Index_Cnt_T_u8 = (pMsgContext->reqDataLen); Index_Cnt_T_u8 < 20U; Index_Cnt_T_u8++)
		{
			Nvm_SystemName_Cnt_u8[Index_Cnt_T_u8] = 0u;
		}
...
void DESC_API_CALLBACK_TYPE ApplDescWrite_DID_98_Repair_Shop_Code_Or_Tester_Serial_Number(DescMsgContext* pMsgContext)
{
	...
	while (Index_Cnt_T_u8 < 10U)
	{
		Nvm_PrevRepairShopCode_Cnt_u8[Index_Cnt_T_u8] = Nvm_RepairShopCode_Cnt_u8[Index_Cnt_T_u8];
		Nvm_RepairShopCode_Cnt_u8[Index_Cnt_T_u8] = pMsgContext->reqData[Index_Cnt_T_u8]; /* PRQA S 0491 */
		Index_Cnt_T_u8++;
	}
	(void)NvM_WriteBlock((NvM_BlockIdType)NVM_BLOCK_SER_REPAIRSHOPCODE, NULL_PTR);
	/* User service processing finished. */
	DescProcessingDone(pMsgContext->iContext);
}
...
void DESC_API_CALLBACK_TYPE ApplDescWrite_DID_99_Programming_Date(DescMsgContext* pMsgContext)
{
	...
	if (((pMsgContext->reqData[2] >= 0x01U) && (pMsgContext->reqData[2] <= 0x09U)) || /* PRQA S 0491 */
		((pMsgContext->reqData[2] >= 0x10U) && (pMsgContext->reqData[2] <= 0x12U))) /* PRQA S 0491 */
	{
		/* Copy date to NvM */
		while (Index_Cnt_T_u8 < 4U)
		{
			Nvm_ProgDate_Cnt_u8[Index_Cnt_T_u8] = pMsgContext->reqData[Index_Cnt_T_u8]; /* PRQA S 0491 */
			Index_Cnt_T_u8++;
		}
...
void DESC_API_CALLBACK_TYPE ApplDescWrite_DID_B4_Manufacturing_Traceability_Characters_new(DescMsgContext* pMsgContext)
{
	...
	while (Index_Cnt_T_u8 < 7U)
	{
		Nvm_ManfTraceability_Cnt_u8[Index_Cnt_T_u8] = pMsgContext->reqData[Index_Cnt_T_u8]; /* PRQA S 0491 */
		Index_Cnt_T_u8++;
	}
...
void DESC_API_CALLBACK_TYPE ApplDescWrite_DID_CB_End_Model_Part_Number(DescMsgContext* pMsgContext)
{
	...
	else
	{
		while (Index_Cnt_T_u8 < 4U)
		{
			Nvm_EndModelPN_Cnt_u8[Index_Cnt_T_u8] = pMsgContext->reqData[Index_Cnt_T_u8]; /* PRQA S 0491 */
			Index_Cnt_T_u8++;
		}
...
void DESC_API_CALLBACK_TYPE ApplDescWrite_DID_DB_End_Model_Part_Number_Alpha_Code(DescMsgContext* pMsgContext)
{
	...
	while (Index_Cnt_T_u8 < 2U)
	{
		Nvm_EndModelPNAC_Cnt_u8[Index_Cnt_T_u8] = pMsgContext->reqData[Index_Cnt_T_u8]; /* PRQA S 0491 */
		Index_Cnt_T_u8++;
	}
...
void DESC_API_CALLBACK_TYPE ApplDescControlCPID_0xAE02_Steering_Assist_Control(DescMsgContext* pMsgContext)
{
	...
	else if ((Rte_SrlComInput_VehicleSpeedValid_Cnt_lgc == FALSE) && (VehSpeedMissing_Cnt_T_lgc != TRUE))
	{
		DescSetExtNegResponse(pMsgContext->iContext, (DescNegResCode)kDescNrcDeviceControlLimitExceeded, D_VEHSPDFLTDETECT_CNT_U16);
	}
	else if ((Rte_SrlComInput_SrlComVehSpd_Kph_f32 > k_CPID02VehSpdLmt_Kph_f32) && (VehSpeedMissing_Cnt_T_lgc != TRUE))
	{
...
void DESC_API_CALLBACK_TYPE ApplDescControlCPID_0xAE03_Steering_Angle_Sensor_Calibration(DescMsgContext* pMsgContext)
{
	...
	if (DescGetStateSecurity_Access() != kDescStateSecurity_AccessCPID_Unlock)
	{
		...
		if(Abs_f32_m(Rte_Sa_DigHwTrqSENT_SrlComHwTrq_HwNm_f32) > k_SteeringAngleSensorCalHwTrqLimit_HwNm_f32)
		{
		...
		else if((Rte_SrlComInput_SrlComVehSpd_Kph_f32 > k_CPID03LockedVehSpdLmt_Kph_f32) && (VehSpeedMissing_Cnt_T_lgc != TRUE))
		{
			DescSetExtNegResponse(pMsgContext->iContext, (DescNegResCode)kDescNrcDeviceControlLimitExceeded, D_VEHSPDABOVECAL_CNT_U16);
		}
		else if (Rte_SrlComInput_SrlComEngOn_Cnt_lgc == TRUE)
		...
		else if(TRUE != Rte_Sa_DigHwTrqSENT_SrlComHwTrqValid_Cnt_lgc)
		{
			DescSetExtNegResponse(pMsgContext->iContext, (DescNegResCode)kDescNrcDeviceControlLimitExceeded, D_FAULTDETECTED_CNT_U16);
		}
		else
		{
	...
	{
		if((Rte_SrlComInput_VehicleSpeedValid_Cnt_lgc == FALSE) && (VehSpeedMissing_Cnt_T_lgc != TRUE))
		{
			DescSetExtNegResponse(pMsgContext->iContext, (DescNegResCode)kDescNrcDeviceControlLimitExceeded, D_VEHSPDFLTDETECT_CNT_U16);
		}		
		else if((Rte_SrlComInput_SrlComVehSpd_Kph_f32 > k_CPID03UnlockedVehSpdLmt_Kph_f32) && (VehSpeedMissing_Cnt_T_lgc != TRUE))
		{
			DescSetExtNegResponse(pMsgContext->iContext, (DescNegResCode)kDescNrcDeviceControlLimitExceeded, D_VEHSPDABOVECAL_CNT_U16);
		}
		else
...
void DESC_API_CALLBACK_TYPE ApplDescControlCPID_0xAE04_Reset_Adaptive_Data(DescMsgContext* pMsgContext)
{
	...
	if (DescGetStateSecurity_Access() != kDescStateSecurity_AccessCPID_Unlock)
	{
		...
		else if((Rte_SrlComInput_SrlComVehSpd_Kph_f32 > k_CPID04LockedVehSpdLmt_Kph_f32) && (VehSpeedMissing_Cnt_T_lgc != TRUE))
		{
			DescSetExtNegResponse(pMsgContext->iContext, (DescNegResCode)kDescNrcDeviceControlLimitExceeded, D_VEHSPDABOVECAL_CNT_U16);
		}
		else if (Rte_SrlComInput_SrlComEngOn_Cnt_lgc == TRUE)
	...
	else
	{
		...
		else if((Rte_SrlComInput_SrlComVehSpd_Kph_f32 > k_CPID04UnlockedVehSpdLmt_Kph_f32) && (VehSpeedMissing_Cnt_T_lgc != TRUE))
		{
			DescSetExtNegResponse(pMsgContext->iContext, (DescNegResCode)kDescNrcDeviceControlLimitExceeded, D_VEHSPDABOVECAL_CNT_U16);
		}
		else
	...
	if (PassChecks_Cnt_T_lgc == TRUE)
	{
		...
		if (D_RESADAPDATAENABLBIT_CNT_U16 == (ControlByte1_Cnt_T_u16 & D_RESADAPDATAENABLBIT_CNT_U16))
		{
			...
			{
			case D_RESETALL_CNT_U16: /* Reset all */
				ActivePull_SCom_Reset();
				LrnEOT_Scom_ResetEOT();
				(void)NtWrapC_TSMit_SCom_GainReset();
				break;
			case D_RESETAPC_CNT_U16: /* Reset Pull Compensation */
				ActivePull_SCom_Reset();
				break;
			case D_RESETEOT_CNT_U16: /* Reset End Of Travel */
				LrnEOT_Scom_ResetEOT();
				break;
			case D_RESETTSMIT_CNT_U16: /* Reset Torque Steer Mitigation */
				(void)NtWrapC_TSMit_SCom_GainReset();
				break;
			default:
				/* Do nothing */
...
void DESC_API_CALLBACK_TYPE ApplDescControlCPID_0xAEFD_System_Basic_Functions(DescMsgContext* pMsgContext)
{
	...
	if (DescGetStateSecurity_Access() != kDescStateSecurity_AccessCPID_Unlock)
	{
		...
		else if((Rte_SrlComInput_SrlComVehSpd_Kph_f32 > k_CPIDFDLockedVehSpdLmt_Kph_f32) && (VehSpeedMissing_Cnt_T_lgc != TRUE))
		{
			DescSetExtNegResponse(pMsgContext->iContext, (DescNegResCode)kDescNrcDeviceControlLimitExceeded, D_VEHSPDABOVECAL_CNT_U16);
		}
		else if(Rte_SrlComInput_SrlComEngOn_Cnt_lgc == TRUE)
	...
	else
	{
		...
		else if((Rte_SrlComInput_SrlComVehSpd_Kph_f32 > k_CPIDFDUnlockedVehSpdLmt_Kph_f32) && (VehSpeedMissing_Cnt_T_lgc != TRUE))
		{
			DescSetExtNegResponse(pMsgContext->iContext, (DescNegResCode)kDescNrcDeviceControlLimitExceeded, D_VEHSPDABOVECAL_CNT_U16);
		}
		else if(Rte_SrlComInput_SrlComEngOn_Cnt_lgc == TRUE)
...
void DESC_API_CALLBACK_TYPE ApplDescUserServiceHandler(DescMsgContext* pMsgContext)
{
	...
	{
		/* Routine ($31) Service */
		ServiceId_Cnt_T_u16 = DescMake16Bit(pMsgContext->reqData[1], pMsgContext->reqData[2]);           /* PRQA S 0491 */
	...
	{
		/* Routine ($31) Service */
		pMsgContext->resDataLen += D_REQMSG_RID_OFFSET_CNT_U16; 	/* PRQA S 2985 */
	}
	else if (DescGetServiceId(pMsgContext->iContext) == D_EPSRESETSRVC_CNT_U16)
	{
		/* Reset Service */
...
void DESC_API_CALLBACK_TYPE ApplDescPreDisableNormalCommunication(vuint8 iContext)
{
	...
	else if ((Rte_SrlComInput_VehicleSpeedValid_Cnt_lgc == TRUE) &&
	         (Rte_SrlComInput_SrlComVehSpd_Kph_f32 > k_DisableNormalComm28VehSpdLmt_Kph_f32) &&
...
void DESC_API_CALLBACK_TYPE ApplDescReadPackVehicle_System_Status(DescMsg pMsg)
{
	...
	BattVoltage_Volt_T_u13p3 = FPM_FloatToFixed_m(Rte_BatteryVoltage_Vecu_Volt_f32, u13p3_T);	/* PRQA S 4395 */
	VehSpd_Kph_T_u16 = (uint16)FPM_FloatToFixed_m(Rte_SrlComInput_SrlComVehSpd_Kph_f32, u16p0_T);	/* PRQA S 4395 */
	...
	for (Index_Cnt_T_u8 = 0u; Index_Cnt_T_u8 < 4U ; Index_Cnt_T_u8++)
	{
		pMsg[Index_Cnt_T_u8] = 0u;		/* PRQA S 0492 */
	}

	/* PRQA S 0492 12 */
	pMsg[0] = DescGetHiByte((uint16)AbsHwPos_HwDeg_T_s14p1); 
	pMsg[1] = DescGetLoByte((uint16)AbsHwPos_HwDeg_T_s14p1);
	pMsg[2] = DescGetLoByte(BattVoltage_Volt_T_u13p3);
	pMsg[3] = DescGetLoByte(VehSpd_Kph_T_u16);
...
vuint8 ApplDescMayEnterProgMode(void)
{
	...
	if ((Rte_SrlComInput_SrlComEngOn_Cnt_lgc == FALSE) &&
		((Rte_SrlComInput_VehicleSpeedValid_Cnt_lgc == FALSE) || (Rte_SrlComInput_SrlComVehSpd_Kph_f32 <= D_MINSESSCHGVEHSPD_KPH_F32)) &&
		((Rte_BatteryVoltage_Vecu_Volt_f32 >= D_MINSESSCHGBATTVOLT_VOLT_F32) && (Rte_BatteryVoltage_Vecu_Volt_f32 <= D_MAXSESSCHGBATTVOLT_VOLT_F32)))
	{
...
void ApplDescGetDtcStatusByMask(vuint16 iterPos, vuint8 statusMask)
{
   ...
   uint32 DemDTCNum_Cnt_T_u32;
	...
	for (Index_Cnt_T_u8 = (uint8)(iterPos+1u); ((Index_Cnt_T_u8 <= (uint8)D_NUMOFDEMEVENTS_CNT_U08) && (FALSE == StatusMatch_Cnt_T_lgc)); Index_Cnt_T_u8++)
	{
...
{
	(*DemDataValueByDataIDBuffer)[0] = DescGetHiByte(Nvm_IgnCntr_Cnt_u16);
	(*DemDataValueByDataIDBuffer)[1] = DescGetLoByte(Nvm_IgnCntr_Cnt_u16);
...
FUNC(Std_ReturnType, APPDESC_CODE) Appl_Dem_CBValByDID_4074(Dem_MaxDataValueType * DemDataValueByDataIDBuffer)
{
	...
	(*DemDataValueByDataIDBuffer)[0] = DescGetHiByte((uint16)StrgWhlAngle_HwDeg_T_s11p4);
	(*DemDataValueByDataIDBuffer)[1] = DescGetLoByte((uint16)StrgWhlAngle_HwDeg_T_s11p4);
	return (Std_ReturnType)E_OK;
}
...
FUNC(Std_ReturnType, APPDESC_CODE) Appl_Dem_CBValByDID_40AC(Dem_MaxDataValueType * DemDataValueByDataIDBuffer)
{
	...
	DigHwTrq_HwNm_T_s7p8 = FPM_FloatToFixed_m(
		Limit_m(
			Rte_Sa_DigHwTrqSENT_HwTorque_HwNm_f32,
			D_DIGHWTRQLOLMT_HWNM_F32,
			D_DIGHWTRQHILMT_HWNM_F32),
		s7p8_T);
	(*DemDataValueByDataIDBuffer)[0] = DescGetHiByte((uint16)DigHwTrq_HwNm_T_s7p8);
	(*DemDataValueByDataIDBuffer)[1] = DescGetLoByte((uint16)DigHwTrq_HwNm_T_s7p8);
	return (Std_ReturnType)E_OK;
}
...
{
	(*DemDataValueByDataIDBuffer)[0] = DescGetLoByte(Rte_CustPerSrvcs_ThermalLimitFlagCntr_Cnt_u08);
	return (Std_ReturnType)E_OK;
}
...
void DESC_API_CALLBACK_TYPE ApplDescRead_Parameter_IdentifierGM_0x40AE_Calculated_System_Temperature(DescMsgContext* pMsgContext)
{
	...
	FiltMeasTemp_DegC_T_s8p7 = FPM_FloatToFixed_m(FiltMeasTemp_DegC_T_f32, s8p7_T);	/* PRQA S 2983 *//* PRQA S 4395 */

	pMsgContext->resData[0] = DescGetHiByte(FiltMeasTemp_DegC_T_s8p7);           /* PRQA S 0491 */
	pMsgContext->resData[1] = DescGetLoByte(FiltMeasTemp_DegC_T_s8p7);          /* PRQA S 0491 */
	pMsgContext->resDataLen = 2u;
	DescProcessingDone(pMsgContext->iContext);
}
...
FUNC(Std_ReturnType, APPDESC_CODE) Appl_Dem_CBValByDID_40BC(Dem_MaxDataValueType * DemDataValueByDataIDBuffer)
{
	...
	{
	  EngineSpeed_Rpm_T_u14p2 = FPM_FloatToFixed_m(Limit_m(Rte_SrlComInput_SrlComEngineSpeed_Rpm_f32, D_ENGSPDLOLMT_RPM_F32, D_ENGSPDHILMT_RPM_F32), u14p2_T);/* PRQA S 4395 */
	}

	(*DemDataValueByDataIDBuffer)[0] = DescGetHiByte(EngineSpeed_Rpm_T_u14p2);
	(*DemDataValueByDataIDBuffer)[1] = DescGetLoByte(EngineSpeed_Rpm_T_u14p2);
	return (Std_ReturnType)E_OK;
}
...
FUNC(Std_ReturnType, APPDESC_CODE) Appl_Dem_CBValByDID_40BD(Dem_MaxDataValueType * DemDataValueByDataIDBuffer)
{
	...
	SrlComVehSpd_Kph_T_u16 = (uint16)FPM_FloatToFixed_m(Rte_SrlComInput_SrlComVehSpd_Kph_f32, u16p0_T);/* PRQA S 4395 */
	SrlComVehSpd_Kph_T_u16 = Limit_m(SrlComVehSpd_Kph_T_u16, D_SRLCOMVEHSPDLOLMT_KPH_U16, D_SRLCOMVEHSPDHILMT_KPH_U16);
	(*DemDataValueByDataIDBuffer)[0] = DescGetLoByte(SrlComVehSpd_Kph_T_u16);
	return (Std_ReturnType)E_OK;
}
...
FUNC(Std_ReturnType, APPDESC_CODE) Appl_Dem_CBValByDID_41D3(Dem_MaxDataValueType * DemDataValueByDataIDBuffer)
{
	...
	VAR(uint16, AUTOMATIC) MtrCurrDaxRef_Amp_T_s14p1;

	MtrCurrDaxRef_Amp_T_f32 = Limit_m(Rte_CurrCmd_MtrCurrDaxRef_Amp_f32, D_41D3MTRCURRMIN_AMP_F32, D_41D3MTRCURRMAX_AMP_F32);
	MtrCurrDaxRef_Amp_T_s14p1 = FPM_FloatToFixed_m(MtrCurrDaxRef_Amp_T_f32, s14p1_T); /* PRQA S 4434 */

	(*DemDataValueByDataIDBuffer)[0] = DescGetHiByte(MtrCurrDaxRef_Amp_T_s14p1);
	(*DemDataValueByDataIDBuffer)[1] = DescGetLoByte(MtrCurrDaxRef_Amp_T_s14p1);
...
FUNC(Std_ReturnType, APPDESC_CODE) Appl_Dem_CBValByDID_41D4(Dem_MaxDataValueType * DemDataValueByDataIDBuffer)
{
	...
	EstPkCurr_Amp_T_u15p1 = FPM_FloatToFixed_m(Limit_m(EstPkCurr_Amp_T_f32, D_ESTPKCURRLOLMT_AMP_F32, D_ESTPKCURRHILMT_AMP_F32), u15p1_T);/* PRQA S 4395 */
	(*DemDataValueByDataIDBuffer)[0] = DescGetHiByte(EstPkCurr_Amp_T_u15p1);
	(*DemDataValueByDataIDBuffer)[1] = DescGetLoByte(EstPkCurr_Amp_T_u15p1);
	return (Std_ReturnType)E_OK;
...
{
	(*DemDataValueByDataIDBuffer)[0] = Limit_m(
			Rte_TrqOvlSta_LKAState_State_enum,
			D_LKASTATELOWLIMIT_CNT_U08,
			D_LKASTATEHIGHLIMIT_CNT_U08
	);
...
{
	(*DemDataValueByDataIDBuffer)[0] = Limit_m(
			Rte_TrqOvlSta_APAState_State_enum,
...
FUNC(Std_ReturnType, APPDESC_CODE) Appl_Dem_CBValByDID_41D7(Dem_MaxDataValueType * DemDataValueByDataIDBuffer)
{
	...
	TrqRqVl_HwNm_T_s15p0 = FPM_FloatToFixed_m(TrqRqVl_HwNm_T_f32,s15p0_T);/* PRQA S 4395 */
...
FUNC(Std_ReturnType, APPDESC_CODE) Appl_Dem_CBValByDID_41D9(Dem_MaxDataValueType * DemDataValueByDataIDBuffer)
{
	...
	PostLimitTorque_MtrNm_s7p8 = (sint16)FPM_FloatToFixed_m(Limit_m(Rte_Ap_AstLmt_SumLimTrqCmd_MtrNm_f32, D_POSTLMTTRQLOLMT_MTRNM_F32, D_POSTLMTTRQHILMT_MTRNM_F32), s7p8_T); /* PRQA S 4395 */
	(*DemDataValueByDataIDBuffer)[0] = DescGetHiByte((uint16)PostLimitTorque_MtrNm_s7p8);
	(*DemDataValueByDataIDBuffer)[1] = DescGetLoByte((uint16)PostLimitTorque_MtrNm_s7p8);
	return (Std_ReturnType)E_OK;
}
...
{
	(*DemDataValueByDataIDBuffer)[0] = Limit_m(Rte_Ap_GMStrtStop_SSState_State_enum,
			D_SSSTATELOWLIMIT_CNT_U08,D_SSSTATEHIGHLIMIT_CNT_U08);
...
FUNC(Std_ReturnType, APPDESC_CODE) Appl_Dem_CBValByDID_446F(Dem_MaxDataValueType * DemDataValueByDataIDBuffer)
{
	...
	SWARTrgtAngRequest_HwDeg_T_s16 = (sint16)(SWARTrgtAngRequest_HwDeg_T_f32 * D_446FCONVERSION_CNTPDEG_F32);	/* PRQA S 4395 */
	SWARTrgtAngRequest_HwDeg_T_s16 = -SWARTrgtAngRequest_HwDeg_T_s16; /* Change Polarity of SWATrgtRequest */

	(*DemDataValueByDataIDBuffer)[0] = DescGetHiByte(SWARTrgtAngRequest_HwDeg_T_s16);
	(*DemDataValueByDataIDBuffer)[1] = DescGetLoByte(SWARTrgtAngRequest_HwDeg_T_s16);
...
void DESC_API_CALLBACK_TYPE ApplDescRead_Parameter_IdentifierGM_0x44E4_Calculated_Rack_Travel(DescMsgContext* pMsgContext)
{
	...
	RackTravel_Mm_T_u8p8 = FPM_FloatToFixed_m(RackTravel_Mm_T_f32, u8p8_T);		/* PRQA S 4445 *//* PRQA S 4395 */
	pMsgContext->resData[0] = DescGetHiByte(RackTravel_Mm_T_u8p8);           /* PRQA S 0491 */
	pMsgContext->resData[1] = DescGetLoByte(RackTravel_Mm_T_u8p8);           /* PRQA S 0491 */
	pMsgContext->resDataLen = 2u;
	DescProcessingDone(pMsgContext->iContext);
}
...
FUNC(Std_ReturnType, APPDESC_CODE) Appl_Dem_CBValByDID_8002(Dem_MaxDataValueType * DemDataValueByDataIDBuffer)
{
	...
	{
		(*DemDataValueByDataIDBuffer)[0] = DescGetLoByte(Rte_SrlComInput_SrlComSysPwrMd_Cnt_enum);
	}
	else
	{
...
FUNC(Std_ReturnType, APPDESC_CODE) Appl_Dem_CBValByDID_8078(Dem_MaxDataValueType * DemDataValueByDataIDBuffer)
{
	...
	BatteryVoltage_Cnt_T_u12p20 = (((uint32)FPM_FloatToFixed_m(Rte_BatteryVoltage_Vecu_Volt_f32, u6p10_T)) * ((uint32)D_BATTVOLTSCALER_ULS_U6P10));
	BatteryVoltage_Cnt_T_u16 = (uint16)(FPM_FixWithRound_m(BatteryVoltage_Cnt_T_u12p20, u12p20_T, u22p10_T));/* PRQA S 2984 *//* PRQA S 4393 *//* PRQA S 4394 */
	BatteryVoltage_Cnt_T_u16 = Limit_m(BatteryVoltage_Cnt_T_u16, D_BATTVOLTLOLMT_CNT_U16, D_BATTVOLTHILMT_CNT_U16);
	(*DemDataValueByDataIDBuffer)[0] = DescGetLoByte(BatteryVoltage_Cnt_T_u16);
	return (Std_ReturnType)E_OK;
}
...
void DESC_API_CALLBACK_TYPE ApplDescRead_Parameter_IdentifierGM_0x819D_Vehicle_Speed_Medium_Resolution(DescMsgContext* pMsgContext)
{
	...
	VehicleSpeed_Kph_T_u10p6 = FPM_FloatToFixed_m(VehicleSpeed_Kph_T_f32, u10p6_T);/* PRQA S 4395 */

	pMsgContext->resData[0] = DescGetHiByte(VehicleSpeed_Kph_T_u10p6);           /* PRQA S 0491 */
	pMsgContext->resData[1] = DescGetLoByte(VehicleSpeed_Kph_T_u10p6);           /* PRQA S 0491 */
	pMsgContext->resDataLen = 2u;
...
void DESC_API_CALLBACK_TYPE ApplDescRead_Parameter_IdentifierGM_0x82C1_Engine_Running_Status(DescMsgContext* pMsgContext)
{
	...
	pMsgContext->resData[0] = CANtemp_Cnt_T_lgc; /* PRQA S 0491 */
	pMsgContext->resDataLen = 1U;
...
FUNC(Std_ReturnType, APPDESC_CODE) Appl_Dem_CBValByDID_C000(Dem_MaxDataValueType * DemDataValueByDataIDBuffer)
{
	...
	{
		(*DemDataValueByDataIDBuffer)[0] = DescGetHiByte(NTCIndex_Cnt_T_u16);
		(*DemDataValueByDataIDBuffer)[1] = DescGetLoByte(NTCIndex_Cnt_T_u16);
...
void DESC_API_CALLBACK_TYPE ApplDescRead_Parameter_IdentifierGM_0xC010_Steering_Angle_Sensor_Status(DescMsgContext* pMsgContext)
{
	...
	pMsgContext->resData[0] = DescGetHiByte(SASLongLermCorrection_Deg_T_s16);           /* PRQA S 0491 *//* PRQA S 2984 */
	pMsgContext->resData[1] = DescGetLoByte(SASLongLermCorrection_Deg_T_s16);           /* PRQA S 0491 */
	pMsgContext->resData[2] = SASInitComplete_Cnt_T_lgc;           /* PRQA S 0491 */
	pMsgContext->resData[3] = (Rte_DigColPs_TrimComp_Cnt_lgc == TRUE) ? 2U : 0U;           /* PRQA S 0491 */
	pMsgContext->resData[4] = DescGetHiByte(SASCenterOffset_Cnt_T_s16);           /* PRQA S 0491 *//* PRQA S 2984 */
	pMsgContext->resData[5] = DescGetLoByte(SASCenterOffset_Cnt_T_s16);           /* PRQA S 0491 */
	pMsgContext->resDataLen = 6u;
...
FUNC(Std_ReturnType, APPDESC_CODE) Appl_Dem_CBValByDID_C050(Dem_MaxDataValueType * DemDataValueByDataIDBuffer)
{
	...
	(*DemDataValueByDataIDBuffer)[0] = 0x00U; /* Only a 4 byte time value */
	(*DemDataValueByDataIDBuffer)[1] = DescGetHiHiByte(TimeoutElapsedTime_mS_T_u32p0);
	(*DemDataValueByDataIDBuffer)[2] = DescGetHiLoByte(TimeoutElapsedTime_mS_T_u32p0);
	(*DemDataValueByDataIDBuffer)[3] = DescGetLoHiByte(TimeoutElapsedTime_mS_T_u32p0);
	(*DemDataValueByDataIDBuffer)[4] = DescGetLoLoByte(TimeoutElapsedTime_mS_T_u32p0);

	return (Std_ReturnType)E_OK;
}

/*  ********************************************************************************
 * Function name:ApplDescRead_Parameter_IdentifierGM_0xC0AF_EPS_System_Status (Service request header:$22 $0 $0 )
 * Description: not available
 * Returns:  nothing
 * Parameter(s):
 *   - pMsgContext->reqData:
...
void DESC_API_CALLBACK_TYPE ApplDescRead_Parameter_IdentifierGM_0xC0AF_EPS_System_Status(DescMsgContext* pMsgContext)
{
	...
	if (Rte_SrlComInput_VehicleSpeedValid_Cnt_lgc == FALSE) 
	{
...
{
	VAR(uint8, AUTOMATIC) CmdResp_Cnt_T_u8 = kDescNrcNone;/* PRQA S 2981 */
	VAR(uint16, AUTOMATIC) PID_Uls_T_u16;

	PID_Uls_T_u16 = DescMake16Bit(pMsgContext->reqData[0], pMsgContext->reqData[1]);/* PRQA S 0491 */
	CmdResp_Cnt_T_u8 = DiagSrvcs_MainHandler(PID_Uls_T_u16, D_SRVCREQTYPEREAD_CNT_U16, &pMsgContext->reqData[D_REQMSG_PID_OFFSET_CNT_U16], pMsgContext->reqDataLen, &pMsgContext->resDataLen);/* PRQA S 0491 */
	pMsgContext->resData = pMsgContext->reqData;
...
/*  ********************************************************************************
 * Function name:ApplDescRead_Parameter_IdentifierNexteer_0xFD12_Current_Mode_Offset_and_Gain (Service request header:$22 $FD $12 )
 * Description: not available
 * Returns:  nothing
 * Parameter(s):
 *   - pMsgContext->reqData:
...
{
	CallFblStart(&CanInitTable_Cnt_M_str);		/* PRQA S 306 ++  */
}
