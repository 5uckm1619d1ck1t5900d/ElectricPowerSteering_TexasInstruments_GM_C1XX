 ...
 *  -------------------------------------------------------------------------------------------------------------------
 *          File:  Ap_DemIf.c
 *     Workspace:  C:/Users/gz324f/Desktop/working/GM_C1XX_EPS_TMS570/GM_C1XX_EPS_TMS570/SwProject/DemIf/autosar
 *     SW-C Type:  Ap_DemIf
 *  Generated at:  Wed Mar 30 13:11:35 2016
 *
 ...
 * -------   -------  --------  ---------------------------------------------------------------------------  ----------
 * 07/29/13  1        LWW       Initial C1xx Synergy Version												    8790
 * 09/10/13  2        LWW       Optimized DEM stored data
 * 11/26/14  3 		  GMN	    Added new enable criteria for the DTCs
 * 03/24/15  4		  GMN       Added mask bit to check CMEC
 * 05/11/15	 5		  GMN		EA3#1230: Added mask to BusOff HS/CE											13336
 * 03/30/16  6		  XL		Fix for EA3#2487: system mode transition
 */
/**********************************************************************************************************************
...
typedef struct CTCInhibitStrType_Structure {
	 ...
	 * \max{FULL}
	 */
	VAR(uint32, AUTOMATIC) CTCNumber_Cnt_u32;
 ...
 *
 * polyspace<MISRA-AC-AGC:8.7:Not a defect:3218>
 */
STATIC CONST(DemIf_CtcInhibitType_Str, AP_DEMIF_CONST) T_CTCTestEnables_Cnt_str[D_NUMOFDEMEVENTS_CNT_U08] = {  /* PRQA S 3218 */
		{0x417654U, &k_CtcInhibitMask417654_Cnt_u16},	/**< System Thermal Error - Temperature High */
		{0x44604BU, &k_CtcInhibitMask44604B_Cnt_u16},	/**< Steering Wheel Angle Sensor - Calibration Not Learned */
		{0x446058U, &k_CtcInhibitMask446058_Cnt_u16},	/**< Steering Wheel Angle Sensor - Hardware Fault */
		{0x44605AU, &k_CtcInhibitMask44605A_Cnt_u16},	/**< Steering Wheel Angle Sensor - Plausibility */
		{0x454500U, &k_CtcInhibitMask454500_Cnt_u16},	/**< Steering Wheel Torque Input Sensor */
		{0x456D00U, &k_CtcInhibitMask456D00_Cnt_u16},	/**< ECU Hardware Performance */
		{0x456E42U, &k_CtcInhibitMask456E42_Cnt_u16},	/**< ECU Swr Performance - Calibration Data Set Not Programmed */
		{0x480003U, &k_CtcInhibitMask480003_Cnt_u16},	/**< Device Power - Voltage Below Threshold */
		{0x480011U, &k_CtcInhibitMask480011_Cnt_u16},	/**< Device Power - Voltage Above Maximum Threshold */
		{0x480012U, &k_CtcInhibitMask480012_Cnt_u16},	/**< Device Power - Voltage Below Minimum Threshold */
 ...
 *********************************************************************************************************************/
	...
	for (i=0U; i<DEM_NVDATA_PATTERN_SIZE; i++)
	{
	...
	for (i=0U; i<DEM_MAX_NUMBER_EVENT_ENTRY; i++)
	{
		if (Dem_NvData.primaryStack[i][0] == 1U)
		{
			...
			for (j=1U; j<=DEM_MAX_SNAPSHOT_RECORD_SIZE; j++)
			{
		...
		else if (Dem_NvData.primaryStack[i][0] == 2U)
		{
			...
			for (j=1U; j<=DEM_MAX_SNAPSHOT_RECORD_SIZE; j++)
			{
				Dem_NvData_BufferOpt.primaryStack[i][j] = Dem_NvData.primaryStack[i][j+DEM_MAX_SNAPSHOT_RECORD_SIZE];
			}
		}
		else
		{
			/* If snapshot counter shows 0 snapshots stored, clear out the snapshot data */
			for (j=0U; j<=DEM_MAX_SNAPSHOT_RECORD_SIZE; j++)
			{
 ...
 *********************************************************************************************************************/
	...
	/* fix for EA3#2487
	 * to make sure after system mode changes DemIf_DelayInhibitPer will run before DemIf_SetEventStatus
	 */
	DemIf_DelayInhibitPer();
...
STATIC FUNC(void, RTE_AP_DEMIF_APPL_CODE) DemIf_DelayInhibitPer(void)
{
	...
	(void)Rte_Read_SrlComSysPwrMd_Cnt_enum(&SysPwrMdSignal_Cnt_T_enum);
...
FUNC(void, RTE_AP_DEMIF_APPL_CODE) DemIf_Init(void)
{
	...
	for (i=0U; i<DEM_MAX_NUMBER_EVENT_ENTRY; i++)
	{
		...
		for (j=1U; j<=DEM_MAX_SNAPSHOT_RECORD_SIZE; j++)
		{
			...
			Dem_NvData.primaryStack[i][j+DEM_MAX_SNAPSHOT_RECORD_SIZE] = Dem_NvData_BufferOpt.primaryStack[i][j];
		}
	...
	for (i=0U; i<=DEM_NUMBER_OF_EVENTS; i++)
	{
