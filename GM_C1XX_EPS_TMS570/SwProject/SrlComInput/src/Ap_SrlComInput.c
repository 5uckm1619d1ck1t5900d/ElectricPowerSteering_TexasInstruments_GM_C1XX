 ...
 *  Generated at:  Thu May 12 14:50:11 2016
 *
 ...
 * -------   -------  --------  ---------------------------------------------------------------------------  ----------
 * 07/29/13  1        LWW       Initial C1xx Synergy Version                                                    8791
 * 09/30/13  2        BWL       Update to new database.
 * 09/30/13  3        BWL       Take defaults on temperature signals when they are missing or invalid.
 * 09/30/13  5,6      BWL       Add new invalid message NTCs and add fault accumulation counters.
 * 03/24/14  7        RM        Add LKA message.
 * 03/26/14  8        BWL       Add new invalid message NTCs and add fault accumulation counters.
 * 04/04/14  9        BWL       Change signal macro usage as required by new message database.
 * 04/25/14  10       BWL       Implement SCIR 01D. Add new messages for APA, and tuning select.
 * 05/12/14  11       BWL       Implement SCIR 01E.
 * 06/17/14  12       BWL       Fix anomalies 6823, 6878, 6462, 6502, and 6879
 * 07/30/14  13       BWL       Fix anomaly 7038, and updates per SCIR 01F
 * 09/02/14  14       JWJ       Added updates to support new SF-47 (TSMit) Component                            12417
 * 10/10/14  16       JWJ       A7370: Corrected tactile feedback protection value for 337 message              12530
 *                              A7380: Corrected wheel angle protection value for 337 message                   12529
 * 10/16/14  17       JWJ       A6863: Added missing condition for APA recoverable fault for missing 0x337      12536
 *                              A7348: Change NTC for missing 0x180 message to 0x1C1                            12547
 *                              A7512: Change NTC for invalid 0x180 message to 0x1C0                            12551
 *                              A7306: Enabled reception of wheel speeds for APA and LKA individually           12545
 *                              A7473: Swapped 0x0C5 message for 0x0C1 message for WIR inputs                   12548
 * 10/22/14  18       JWJ       A7534: Corrected polarity on 0x337 message                                      12569
 * 11/07/14  19       JWJ       A6954: Added default write for engine temp invalid or missing                   12605
 *                              A6956: Added default write for ambient temp invalid or missing                  12606
 *                              A7557: Removed two needed cals for ambient/engine temp invalid diag str         12634
 *                              A7543: Corrected default value for wheel speeds being written in LKA disable    12633
 *                              A7529: Added polling function for 0x0D3 message                                 12620
 *                              A7571: Added a new bit for invalid Min/Max secure speed for APA fault           12675
 *                              A7495: Corrected NTC pre-passing function calls for APA/LKA-only NTCs           12628
 *                              A6538: Remove duplicate calibrations                                            12598
 *                              A6985: Added default value for shift level position when message is missing     12609
 *                              A6981: Select system power mode based upon ERA when 1F1 message missing         12607
 *                              A6852: Modified criteria to pass NTC 170 - shared with non-LKA signals          12603
 * 11/24/14  21       JWJ       A7521: Corrected the  CMEC decrement condition to work with SPM transition      12630
 * 01/26/15  22       JWJ       A7783: Corrected reception of transmission torque signal group                  12933
 *                              ICR-4702: Cyber security updates - primarily CMEC criteria changes              12851
 * 02/02/15  23       JWJ       A7556: Correct default values for x180 Invalid signals                          12847
 *                              A7806: Updated logic for NTC 170 Parameter bit 1                                12937
 *                              A7772: Enabled Secure Vehicle Speed calculation for ESC only condition          12938
 *                              A7719: Corrected proccessing for DMC in x232 message                            12882
 *                              A7746: Updated the list of pre-passed NTC based upon advanced feature states    12887
 * 05/14/15  25       GMN       EA3#1246: APA Non Recovery faults Sensor Failure                                13340
 *                              EA3#1250: APA Non Recovery faults Sensor Failure                                13341
 *                              EA3#1255: APA Non Recovery faults CAN faults                                    13342
 * 06/11/15  26       GMN       Change GM MEC decrement logic
 *                              APA Non Recovery faults input changed from OP DISABLE to DigMgr Non Recovery
 *                              faults output
 * 07/09/15  27       GMN       APA Non Recovery faults logic changed (more calibrations)                       13454
 *                              NTC 121 status clear                                                           13451
 *                              Default value for SysPwMd signal when message is missing or bus off             13504
 * 12/12/15  29       CP        LKAFault, APA nonrec fault calibration are disabled for invalid min veh speed
 *                              EA3#2632 and datatype corrected for IlGetRxVhDynCntlSysSt EA3#3128     
 * 04/10/16  30       CP        EA3#5387: APA temp inhibit with only 1 invalid 348/34A message  
 * 05/03/16  31       CP        EA3#226 ESC should be enabled for Missing and Invalid NTCs of CE Msg 180,182 
                                EA3#2425,EA3#2485: Missing message faults being set while HS/CE busoff is active
 * 05/03/16  31       CP        EA3#226 ESC should be enabled for Missing and Invalid NTCs of CE Msg 180,182 
                                EA3#1974: EBCM to ESC does not set CE message to perm fail 
 * 07/26/16  35       CP        EA3#9182: NTC 180(Invalid Vehicle Dynamics Yaw Rate) not being set 
								when CMEC is 0< CMEC < FF
 */
/**********************************************************************************************************************
...
#define D_NORMALDMC_CNT_U16								1U
#define D_SPORTDMC_CNT_U16								2U
#define D_OFFROADDMC_CNR_U16							3U
#define D_TOWHAULDMC_CNT_U16							4U
#define D_DEFAULTPERS_CNT_U16							0U
#define D_NORMALPERS_CNT_U16							0U
#define D_SPORTPERS_CNT_U16								1U
#define D_OFFRAODPERS_CNT_U16							2U
#define D_TOWHAULPERS_CNT_U16							3U
...
#define D_ESCFAULT_F1F2FAULTPRESENT_CNT_U16				((uint16)1u << 2u)
...
/* This section is changed from NoInit Cleared section of App9 to Init 32 region of App9
in build 06.02.02 to free memory in noint app09 for future changes */

#define SRLCOMINPUT_START_SEC_VAR_INIT_32
#include "MemMap.h" /* PRQA S 5087 */
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) LastPlsChngTimeL_mS_M_u32; /* PRQA S 3218 */
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) LastPlsChngTimeR_mS_M_u32; /* PRQA S 3218 */
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) InvStartTimeL_mS_M_u32; /* PRQA S 3218 */
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) InvStartTimeR_mS_M_u32; /* PRQA S 3218 */
STATIC VAR(float32, AP_SRLCOMINPUT_VAR_INIT) OldRawWhlFreqL_Hz_M_f32; /* PRQA S 3218 */
STATIC VAR(float32, AP_SRLCOMINPUT_VAR_INIT) OldRawWhlFreqR_Hz_M_f32; /* PRQA S 3218 */
STATIC VAR(float32, AP_SRLCOMINPUT_VAR_INIT) PulseScale_RevpCnt_M_f32;
STATIC VAR(float32, AP_SRLCOMINPUT_VAR_INIT) WhlTstmpRes_SecpCnt_M_f32;
STATIC VAR(float32, AP_SRLCOMINPUT_VAR_INIT) VehSpdAvgNDrvn_Kph_M_f32;
STATIC VAR(float32, AP_SRLCOMINPUT_VAR_INIT) ManualVehSpd_Kph_M_f32;
STATIC VAR(float32, AP_SRLCOMINPUT_VAR_INIT) MaxSecVehSpd_Kph_M_f32; /* PRQA S 3218 */
STATIC VAR(float32, AP_SRLCOMINPUT_VAR_INIT) MinSecVehSpd_Kph_M_f32; /* PRQA S 3218 */

/* Invalid message timers */
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) AmbTempVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) EngTempVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) LatAccVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) YawRateVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) VehSpdVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) VehicleDynamicsESCHybCEVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) ParkAssistParallelCEVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) AntilockTCbrakeVldStartTime_mS_M_u32p0;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) RedntVSEActARCVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) RedntVSEActVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) ABSActvProtPValVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) ABSActvProtARCVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) ABSActvProtVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) VSEActVldStartTime_mS_M_u32p0;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) TCSysEVldStartTime_mS_M_u32p0;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) ABSFldVldStartTime_mS_M_u32p0;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) VhDynCVldStartTime_mS_M_u32p0;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) LKATqOvrDltCmdPrtVlVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) LKATqOvrDltCmdRCVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) ActAxleTrqVldStartTime_mS_M_u32; /* PRQA S 3218 */
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) TrnsShftLvrPosVldStartTime_mS_M_u32; /* PRQA S 3218 */
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) EngSpdStatlVldStartTime_mS_M_u32; /* PRQA S 3218 */
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) ESCChksmVldStartTime_mS_M_u32; /* PRQA S 3218 */
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) ESCRollCountVldStartTime_mS_M_u32; /* PRQA S 3218 */

/* Msg loss timers */
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg1E9Loss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg214Loss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg232Loss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg0C9Loss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg0C1Loss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg0D3Loss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg1F1Loss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg1F5Loss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg3E9Loss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg500Loss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg348CELoss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg348HSLoss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg180HSLoss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg34ACELoss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg34AHSLoss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg337Loss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg337LossAPARec_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg337LossAPANonRec_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg182Loss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg3F1Loss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg4C1Loss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg17DLoss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg180CELoss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg180HSLKALoss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg348HSLKALoss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg348CELKALoss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg34AHSLKALoss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg34ACELKALoss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg1E9LKALoss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg214LKALoss_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) Msg17DLKALoss_mS_M_u32;

/* Wheel Ground Velocity Stuck Signal Timers */
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyRtDrvnCEStuck_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyLftDrvnCEStuck_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyRtNnDrvnCEStuck_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyLftNnDrvnCEStuck_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyRtDrvnHSStuck_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyLftDrvnHSStuck_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyRtNnDrvnHSStuck_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyLftNnDrvnHSStuck_mS_M_u32;

/* Wheel Ground Velocity Validity Timers */
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyRtDrvnCEVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyLftDrvnCEVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyRtNnDrvnCEVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyLftNnDrvnCEVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyRtDrvnHSVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyLftDrvnHSVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyRtNnDrvnHSVldStartTime_mS_M_u32;
STATIC VAR(uint32, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyLftNnDrvnHSVldStartTime_mS_M_u32;
#define SRLCOMINPUT_STOP_SEC_VAR_INIT_32
#include "MemMap.h" /* PRQA S 5087 */

/* This section is changed from NoInit Cleared section of App9 to Init 32 region of App9
in build 06.02.02 to free memory in noint app09 for future changes */
#define SRLCOMINPUT_START_SEC_VAR_INIT_16
#include "MemMap.h" /* PRQA S 5087 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) AmbTempVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) EngTempVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) LatAccVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) YawRateVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) VehSpdVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) VehicleDynamicsESCHybCEVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) ParkAssistParallelCEVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyRtDrvnHSVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyLftDrvnHSVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyRtNnDrvnHSVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyLftNnDrvnHSVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyRtDrvnCEVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyLftDrvnCEVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyRtNnDrvnCEVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyLftNnDrvnCEVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) RedntVSEActARCVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) RedntVSEActVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) ABSActvProtPValVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) ABSActvProtARCVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) ABSActvProtVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) VSEActVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) TCSysEVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) ABSFldVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) VhDynCVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) LKATqOvrDltCmdPrtVlVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) LKATqOvrDltCmdRCVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) ActAxleTrqVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) TrnsShftLvrPosVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) EngSpdStatVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) ESCChksmVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) ESCRollCountVldAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg1E9LossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg214LossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg232LossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg0C9LossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg0C1LossAccum_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg0D3LossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg1F1LossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg1F5LossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg3E9LossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg500LossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg348CELossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg348HSLossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg180HSLossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg34ACELossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg34AHSLossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg337LossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg337LossAccumAPA_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg337InvalidAccumAPA_Cnt_M_u16;	/* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg182LossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg3F1LossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg4C1LossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg17DLossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) Msg180CELossAccum_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) OldSeqNumL_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) OldSeqNumR_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) OldWhlDistTstmL_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) OldWhlDistTstmR_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) OldWhlPCntrL_Cnt_M_u16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) OldWhlPCntrR_Cnt_M_u16; /* PRQA S 3218 */

STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyRtDrvnCE_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyLftDrvnCE_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyRtNnDrvnCE_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyLftNnDrvnCE_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyRtDrvnHS_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyLftDrvnHS_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyRtNnDrvnHS_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WhlGrndVlctyLftNnDrvnHS_Cnt_M_u16;

STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) PrevLKARollingCounter_Cnt_M_u16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) LKAInhibit_Cnt_M_b16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) LKAFault_Cnt_M_b16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) StartStopFault_Cnt_M_b16;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) ESCFault_Cnt_M_b16;
volatile STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) LKAFaultLatch_Cnt_D_b16; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) PrevESCRollingCounter_Cnt_M_u16;

STATIC VAR(uint8, AP_SRLCOMINPUT_VAR_INIT) APARecoverableFaults_Cnt_M_b08;
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) APANonRecoverableFaults_Cnt_M_b16;
volatile STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) APANonRecFaultsLatch_Cnt_D_b16; /* PRQA S 3218 */
volatile STATIC VAR(uint8, AP_SRLCOMINPUT_VAR_INIT) APARecFaultsLatch_Cnt_D_b08; /* PRQA S 3218 */
STATIC VAR(uint16, AP_SRLCOMINPUT_VAR_INIT) WIRFltStatusAcc_Cnt_M_u16; /* PRQA S 3218 */
#define SRLCOMINPUT_STOP_SEC_VAR_INIT_16
#include "MemMap.h" /* PRQA S 5087 */
...
STATIC VAR(boolean, AP_SRLCOMINPUT_VAR_NOINIT) SrlComInput_Message1F1Received; /* PRQA S 3218 */
 ...
 *********************************************************************************************************************/
	...
	{
		(void)Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_MissingMsg_AB, 0x00u, NTC_STATUS_PASSED); /* 0x348 HS Missing */
		(void)Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_InvalidMsg_AB, 0x00u, NTC_STATUS_PASSED); /* 0x348 HS Invalid */
 ...
 *********************************************************************************************************************/
	...
	(void)Rte_Call_SrlComInput_Per2_CP0_CheckpointReached();
	...
	BusOffHSPresent_Cnt_T_lgc = Rte_IRead_SrlComInput_Per2_BusOffHS_Cnt_lgc();
	BusOffCEPresent_Cnt_T_lgc = Rte_IRead_SrlComInput_Per2_BusOffCE_Cnt_lgc();

	/* Poll for messages on the HS bus. */
	...
	/* Poll for messages on the CE bus. */
	...
	if ((TRUE == IlNwmStateNormalCommHalted(NmStateChannel0_Cnt_T_enum)) ||
			(TRUE == BusOffHSPresent_Cnt_T_lgc))
	{
		SrlComInput_SCom_ResetBus1Timers();
	}

	if ((TRUE == IlNwmStateNormalCommHalted(NmStateChannel1_Cnt_T_enum)) ||
			(TRUE == BusOffCEPresent_Cnt_T_lgc))
	{
		SrlComInput_SCom_ResetBus2Timers();
	}

	/* In case of no communication / bus off, the SysPwMd signal should go to a default state */
	if (TRUE == IlNwmStateNoCommunication(NmStateChannel0_Cnt_T_enum))
	{
		(void)Rte_Write_SrlComSPMOn_Cnt_lgc(FALSE);
	}
	...
	if (RxMsgSrlComSvcDft_Cnt_T_lgc == FALSE)
	{
		...
		{
			(void)Rte_Write_SrlComLatAccel_g_f32(k_LatAccDflt_MpSecSqrd_f32/ D_MPSECSQRDTOGRAVITYACCEL_ULS_F32);
		...
		if ((TRUE == k_LKAMfgEnable_Cnt_lgc) || (TRUE == k_APAMfgEnable_Cnt_lgc) || (TRUE == k_ESCMfgEnable_Cnt_lgc))
		{
			...
			(void)DtrmnNTCTimeout(&Msg348HSLoss_mS_M_u32, k_Msg348HSTimeOut_mS_u16p0, &Msg348HSLossAccum_Cnt_M_u16,
			...
			(void)DtrmnNTCTimeout(&Msg34AHSLoss_mS_M_u32, k_Msg34AHSTimeOut_mS_u16p0, &Msg34AHSLossAccum_Cnt_M_u16,
		...
		else
		{
			...
			(void)Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_MissingMsg_P, 0x00u, NTC_STATUS_PASSED);                  /* 0x1F5 HS Missing */
			(void)Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_MissingMsg_ParkAssistParallel, 0x00u, NTC_STATUS_PASSED); /* 0x337 CE Missing */
			(void)Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_InvalidMsg_P, 0x00u, NTC_STATUS_PASSED);                  /* 0x1F5 HS Invalid */
			(void)Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_InvalidMsg_ParkAssistParallel, 0x00u, NTC_STATUS_PASSED); /* 0x337 CE Invalid */
		}
		...
		if (TRUE == k_ESCMfgEnable_Cnt_lgc)
		{
			...
			if(TRUE == DtrmnNTCTimeout(&Msg180CELoss_mS_M_u32, k_Msg180CETimeOut_mS_u16p0,
						&Msg180CELossAccum_Cnt_M_u16, k_Msg180CETimeOutDiag_Cnt_str,
						NTC_Num_MissingMsg_AG, D_FALSE_CNT_LGC, 0x01u))
			{
				ESCFault_Cnt_M_b16 |= D_ESCFAULT_MISSING180CE_CNT_U16;
			}
			
			if(TRUE == F1orF2FaultPresent_Cnt_T_lgc)
			{
				ESCFault_Cnt_M_b16 |= D_ESCFAULT_F1F2FAULTPRESENT_CNT_U16;
			}
			else
			{
				ESCFault_Cnt_M_b16 &= (uint16)~D_ESCFAULT_F1F2FAULTPRESENT_CNT_U16;
			}

			/* If no bits in the ESC Fault bit mask are set, then it remains false. */
		...
		else
		{
			...
			(void)Rte_Write_ESCRequest_Cnt_lgc(FALSE);

			(void)Rte_Call_SystemTime_GetSystemTime_mS_u32(&Msg180CELoss_mS_M_u32);
		...
		(void)DtrmnNTCTimeout(&Msg3E9Loss_mS_M_u32, k_Msg3E9TimeOut_mS_u16p0,
				...
				NTC_Num_MissingMsg_R, D_FALSE_CNT_LGC, 0x01u);
...
{
	VAR(RT_PPEI_NonDrivn_Whl_Rotationl_Stat, AUTOMATIC) Data;
	VAR(float32, AUTOMATIC) PulseScale_RevpCnt_T_f32;
	VAR(float32, AUTOMATIC) WhlTstmpRes_SecpCnt_T_f32;
	...
	Data.WhlRotStatLft.WRSSeqNum = IlGetRxWRSLDSeqNum();
	Data.WhlRotStatLft.WRSWhlDisTpRC = IlGetRxWRSLDWhlDisTpRC();
	Data.WhlRotStatLft.WRSWhlDistVal = IlGetRxWRSLDWhlDistVal();
	Data.WhlRotStatLft.WRSWhlRotStatRst = IlGetRxWRSLDWhlRotStatRst();
	Data.WhlRotStatLft.WRSWhlDistTstm = IlGetRxWRSLDWhlDistTmstm();
	Data.WhlRotStatLft.WRSWhlDistPCntr = IlGetRxWRSLDWhlDistPlsCntr();

	Data.WhlRotStatRght.WRSSeqNum = IlGetRxWRSRDSeqNum();
	Data.WhlRotStatRght.WRSWhlDisTpRC = IlGetRxWRSRDWhlDisTpRC();
	Data.WhlRotStatRght.WRSWhlDistVal = IlGetRxWRSRDWhlDistVal();
	Data.WhlRotStatRght.WRSWhlRotStatRst = IlGetRxWRSRDWhlRotStatRst();
	Data.WhlRotStatRght.WRSWhlDistTstm = IlGetRxWRSRDWhlDistTmstm();
	Data.WhlRotStatRght.WRSWhlDistPCntr = IlGetRxWRSRDWhlDistPlsCntr();
	...
	if (RxMsgSrlComSvcDft_Cnt_T_lgc == FALSE)
	{
		...
		else
		{
			...
			RawWhlFreqL_Hz_T_f32 = ProcessWhlStatus(Data.WhlRotStatLft,
			...
			RawWhlFreqR_Hz_T_f32 = ProcessWhlStatus(Data.WhlRotStatRght,
...
FUNC(void, RTE_AP_SRLCOMINPUT_APPL_CODE) SrlComInput_MsgProcess_RxChassisGeneralStatus1_x1E9(CanReceiveHandle rcvObject)
{
	...
	Data.VSELatAcc = IlGetRxVSELatAcc();
	Data.VSELatAccV = IlGetRxVSELatAccV();
	Data.VehDynYawRate = IlGetRxVehDynYawRate();
	Data.VehDynYawRateV = IlGetRxVehDynYawRateV();
	...
	if (RxMsgSrlComSvcDft_Cnt_T_lgc == FALSE)
	{
		if (D_VALID_CNT_U08 == Data.VSELatAccV)
		{
			if ((Data.VSELatAcc) > D_LATACCELYAWRATEMAXVAL_CNT_U16)
			{
				SrlComLatAcc_MpSecSqrd_T_f32 = ((float32)(((sint16)((Data.VSELatAcc) - (D_SIGNEDSIGNALOFFSET_CNT_U16)))) * D_VEHSTBLATACCELSF_MPSECSQRD_F32);
			}
			else
			{
				SrlComLatAcc_MpSecSqrd_T_f32 = (float32)Data.VSELatAcc * D_VEHSTBLATACCELSF_MPSECSQRD_F32;
			}
			...
			SrlComLatAcc_g_T_f32 = (float32)(SrlComLatAcc_MpSecSqrd_T_f32/ D_MPSECSQRDTOGRAVITYACCEL_ULS_F32);
			(void)Rte_Write_SrlComLatAccel_g_f32(SrlComLatAcc_g_T_f32);
		...
		if (D_VALID_CNT_U08 == Data.VehDynYawRateV)
		{
			if ((Data.VehDynYawRate) > D_LATACCELYAWRATEMAXVAL_CNT_U16)
			{
				SrlComYawRate_DegpSec_T_f32 = ((float32)(((sint16)((Data.VehDynYawRate) - (D_SIGNEDSIGNALOFFSET_CNT_U16)))) * D_VEHDYNYAWRATESF_DEGPSEC_F32);
			}
			else
			{
				SrlComYawRate_DegpSec_T_f32 = (float32)Data.VehDynYawRate * D_VEHDYNYAWRATESF_DEGPSEC_F32;
			}
			...
			(void)NTCCheckClear(&YawRateVldStartTime_mS_M_u32, &YawRateVldAccum_Cnt_M_u16, &k_YawRateValDiag_Cnt_str, NTC_Num_InvalidMsg_Y);
				                        ...
				                        NTC_Num_InvalidMsg_Y, D_FALSE_CNT_LGC, 0x01u))
		{
			(void)Rte_Write_SrlComYawRate_DegpS_f32(k_YawRateDflt_DegpSec_f32);
			SrlComInput_Message1E9PresentAndValid_Cnt_M_lgc = FALSE;
		}
		...
		{
			/* Message is missing, but not for long enough to set the NTC. */
		}
...
{
	VAR(RT_PPEI_Platform_General_Status, AUTOMATIC) Data;
	...
	SrlComInput_Message1F1Received = TRUE;
	IlGetRxBkupPwrModeMstrGroup();

	Data.SysPwrMode = IlGetRxSysPwrMd();
	Data.SysBkupPwrMdEn = IlGetRxSysBkupPwrMdEn();
	Data.SysBkUpPwrMd = IlGetRxSysBkUpPwrMd();
	Data.BkupPwrModeMstrVDA = IlGetRxBkupPwrModeMstrVDA();
	...
	if (RxMsgSrlComSvcDft_Cnt_T_lgc == FALSE)
	{
		if (FALSE == Data.SysBkupPwrMdEn)
		{
			PwrMode_Cnt_T_u08 = Data.SysPwrMode;
		 	if (Run == Data.SysPwrMode)
		 	{
		...
		else if (TRUE == Data.BkupPwrModeMstrVDA)
		{
			PwrMode_Cnt_T_u08 = Data.SysBkUpPwrMd;
			if (Run == Data.SysBkUpPwrMd)
		 	{
				SPMOn_Cnt_T_lgc = TRUE;
				SPMForCTCInhibit_Cnt_T_lgc = TRUE;
				SrlComInput_PrevRunMode_Cnt_M_lgc = TRUE;
		...
		Data.SysPwrMode = Limit_m(Data.SysPwrMode, D_SYSPWRMDMIN_CNT_U08, D_SYSPWRMDMAX_CNT_U08);
		(void)Rte_Write_SrlComSPMOn_Cnt_lgc(SPMOn_Cnt_T_lgc);
		(void)Rte_Write_SrlComSysPwrMd_Cnt_enum(Data.SysPwrMode);
...
FUNC(void, RTE_AP_SRLCOMINPUT_APPL_CODE) SrlComInput_MsgProcess_RxBrakePedalDriverStatusHS_x214(CanReceiveHandle rcvObject)
{
				...
				(ABSActvProtARC_Cnt_T_u08 != ((ABSActvProtARC_Cnt_M_u08 + 1u) & 0x03u)))
	{
		...
		MessageValid_Cnt_T_lgc = FALSE;
		(void)DtrmnNTCTimeout(&ABSActvProtARCVldStartTime_mS_M_u32, k_ABSActvProtARCVldTimeOut_mS_u16p0,
								   &ABSActvProtARCVldAccum_Cnt_M_u16, k_ABSActvProtARCVldDiag_Cnt_str,
								   NTC_Num_InvalidMsg_X, D_FALSE_CNT_LGC, 0x04u);
	}
...
{
	VAR(boolean, AUTOMATIC) RxMsgSrlComSvcDft_Cnt_T_lgc;
	VAR(uint8, AUTOMATIC) StgSysPerfMdRq_Cnt_T_u08;

	StgSysPerfMdRq_Cnt_T_u08 = IlGetRxStgSysPerfMdRq();

	(void)Rte_Read_RxMsgsSrlComSvcDft_Cnt_lgc(&RxMsgSrlComSvcDft_Cnt_T_lgc);

	if (RxMsgSrlComSvcDft_Cnt_T_lgc == FALSE)
	{
		switch(StgSysPerfMdRq_Cnt_T_u08)
		{
		case D_NORMALDMC_CNT_U16:
			(void)Rte_Write_DesiredTunPers_Cnt_u16(D_NORMALPERS_CNT_U16);
			break;
		case D_SPORTDMC_CNT_U16:
			(void)Rte_Write_DesiredTunPers_Cnt_u16(D_SPORTPERS_CNT_U16);
			break;
		case D_OFFROADDMC_CNR_U16:
			(void)Rte_Write_DesiredTunPers_Cnt_u16(D_OFFRAODPERS_CNT_U16);
			break;
		case D_TOWHAULDMC_CNT_U16:
			(void)Rte_Write_DesiredTunPers_Cnt_u16(D_TOWHAULPERS_CNT_U16);
			break;
		default:
...
{
	VAR(RT_PPEI_Vehicle_Speed_and_Distance, AUTOMATIC) Data;
	VAR(boolean, AUTOMATIC) RxMsgSrlComSvcDft_Cnt_T_lgc;

	IlGetRxVehSpdAvgNDrvnGroup();

	Data.VehSpdAvgNonDrvnGroup.VehSpdAvgNDrvn = IlGetRxVehSpdAvgNDrvn();
	Data.VehSpdAvgNonDrvnGroup.VehSpdAvgNDrvnV = IlGetRxVehSpdAvgNDrvnV();

	(void)Rte_Read_RxMsgsSrlComSvcDft_Cnt_lgc(&RxMsgSrlComSvcDft_Cnt_T_lgc);

	if (RxMsgSrlComSvcDft_Cnt_T_lgc == FALSE)
	{
		if (D_VALID_CNT_U08 == Data.VehSpdAvgNonDrvnGroup.VehSpdAvgNDrvnV)
		{
		...
		VehSpdAvgNDrvn_Kph_M_f32 = ((float32)Data.VehSpdAvgNonDrvnGroup.VehSpdAvgNDrvn) * D_VEHSPDAVGNDRVNSF_KPH_F32;
...
{
	VAR(RT_Platform_Eng_Cntrl_Requests, AUTOMATIC) Data;
	...
	Data.OtsAirTmpCrVal = IlGetRxOtsAirTmpCrVal();
	Data.OtsAirTmpCrValV = IlGetRxOtsAirTmpCrValV();
	Data.OtsAirTmpCrValMsk = IlGetRxOtsAirTmpCrValMsk();
	...
	if (RxMsgSrlComSvcDft_Cnt_T_lgc == FALSE)
	{
		...
		if ((D_VALID_CNT_U08 == Data.OtsAirTmpCrValV) && (D_USEDATA_CNT_U08 == Data.OtsAirTmpCrValMsk))
		{
			SrlComAmbTemp_DegC_T_f32 = ((float32)Data.OtsAirTmpCrVal * D_OUTAIRTEMPCORRVALSF_DEGC_F32) - D_OUTAIRTEMPCORRVALOFFSET_DEGC_F32;
...
{
	VAR(RT_Engine_General_Status_4, AUTOMATIC) Data;
	...
	Data.EngCltTmp = IlGetRxEngCltTmp();
	Data.EngCltTmpV = IlGetRxEngCltTmpV();
	...
	{
		/* TODO: Make common signal scaling function */
		if (D_VALID_CNT_U08 == Data.EngCltTmpV)
		{
			SrlComEngTemp_DegC_T_f32 = (float32)Data.EngCltTmp - D_ENGCLTTEMPOFFSET_DEGC_F32;
...
{
	VAR(RT_Wheel_Pulses_HS, AUTOMATIC) Data;
	VAR(float32, AUTOMATIC) PulseScale_RevpCnt_T_f32;
	VAR(float32, AUTOMATIC) WhlTstmpRes_SecpCnt_T_f32;
	VAR(boolean, AUOTMATIC) WIREnabled_Cnt_T_lgc;
	VAR(boolean, AUTOMATIC) RxMsgSrlComSvcDft_Cnt_T_lgc;

	Data.WhlRotStatTmstmpRes = IlGetRxWhlRotStatTmstmpRes();
	Data.WhlPlsPerRevNonDrvn = IlGetRxWhlPlsPerRevNonDrvn_0();
	...
	if (RxMsgSrlComSvcDft_Cnt_T_lgc == FALSE)
	{
		...
		{
			/* Process Data */
			if ((0u != Data.WhlPlsPerRevNonDrvn) && (0u != Data.WhlRotStatTmstmpRes))
			{
				PulseScale_RevpCnt_T_f32 = D_ONE_ULS_F32/(float32)Data.WhlPlsPerRevNonDrvn;
				WhlTstmpRes_SecpCnt_T_f32 = (((float32)Data.WhlRotStatTmstmpRes) * D_2MS_SEC_F32 * D_1MICROSEC_SEC_F32);
...
FUNC(void, RTE_AP_SRLCOMINPUT_APPL_CODE) SrlComInput_MsgProcess_RxVehicleDynamicsESCHybCE_x182(CanReceiveHandle rcvObject)
{
	...
	VAR(float32, AUTOMATIC) VSELongAccLoRes_KphpS_T_f32;
	...
	if (RxMsgSrlComSvcDft_Cnt_T_lgc == FALSE)
	{
		...
		if (D_VALID_CNT_U08 == VSELongAccLoResV_Cnt_T_u08)
		{
			...
			/* This rather wonky looking conversion is to change units from m/(2^2) to Kph/s */
			VSELongAccLoRes_KphpS_T_f32 = VSELongAccLoRes_MpSecSqrd_T_f32 * D_LONACCELCONVFACTOR_KPHSPM_F32;
			(void)Rte_Write_SrlComVehicleLonAccel_KphpS_f32(VSELongAccLoRes_KphpS_T_f32);
		}
...
STATIC FUNC(void, RTE_AP_SRLCOMINPUT_APPL_CODE) SrlComInput_DecrementCMEC(void)
{
    ...
    {
        Nvm_CMEC_Cnt_u8 -= 1U;
        (void)NvM_SetRamBlockStatus(NVM_BLOCK_SER_CUSTMEC, TRUE);
    }
}
